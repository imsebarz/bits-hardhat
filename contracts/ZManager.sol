...
interface IERC20 {
    ...
    function manager_mint(address receiver_, uint256 amount_) external;

    function burn(address _redeemAddress, uint256 _amount) external;
}

contract OW3NFTs is ERC721Enumerable {
    uint256 public totalMinted; //Cantidad total Minteada
    ...
    mapping(address => mapping(uint256 => bool)) address_propertyID2state; //Relación entre una dirección y una medalla devuelve true si la tiene o false si no
    mapping(address => mapping(uint256 => bool)) public address_prizeID2state; //Relación entre una dirección y un premio devuelve true si la tiene o false si no
    mapping(uint256 => uint256) public NFTID2propertyID; //Cada NFT va enumerado y se asigna un numero de medalla para relacionar su metadata
    mapping(uint256 => uint256) public propertyID2tokenAmount; //mapeo para relacionar la medalla con el premio a entregar
    struct prize {
        string name;
        uint256 tokens_to_redeem_prize;
        uint256[] properties_required;
    }
    ...
    function mintproperty(address receiver_, uint256 _property_ID)
        public
        onlyAutorized
    {
    ...
    function mint_tokens(address receiver_, uint256 amount_)
        public
        onlyAutorized
    {
    ...
    function change_propertyID_URI(uint256 property_Id, string memory _newURI)
        public
        onlyAutorized
    {
